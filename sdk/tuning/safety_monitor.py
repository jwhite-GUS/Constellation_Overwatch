...
class SafetyMonitor:
    ...
    def _check_attitude_limits(self, attitude: Dict[str, float]):
        ...
        if abs(roll) > self.limits.max_roll:
            severity = SafetyLevel.CRITICAL if abs(roll) > self.limits.max_roll * 1.2 else SafetyLevel.WARNING
            self._add_violation(
        ...
        if abs(pitch) > self.limits.max_pitch:
            severity = SafetyLevel.CRITICAL if abs(pitch) > self.limits.max_pitch * 1.2 else SafetyLevel.WARNING
            self._add_violation(
                SafetyViolationType.ATTITUDE_LIMIT,
                severity,
                abs(pitch),
    ...
    def _check_rate_limits(self, rates: Dict[str, float]):
        ...
        for axis, rate, limit in rate_checks:
            if abs(rate) > limit:
                severity = SafetyLevel.CRITICAL if abs(rate) > limit * 1.5 else SafetyLevel.WARNING
                self._add_violation(
    ...
    def _check_oscillations(self):
        ...
        for axis in ['roll', 'pitch', 'yaw']:
            ...
            oscillation_freq = self.oscillation_detector.detect_oscillation_frequency(axis_data)
    ...
    def get_safety_status(self) -> Dict:
        ...
        return {
            ...
            'recent_violations': len([v for v in self.violations if time.time() - v.timestamp < 10.0]),
    ...
    def _monitor_loop(self):
        ...
        while self._monitor_running:
            try:
                ...
                # Check for sensor timeouts
                if current_time - self.last_sensor_update > self.limits.sensor_timeout * 2:
                    self._add_violation(
                        SafetyViolationType.SENSOR_FAILURE,
                        SafetyLevel.CRITICAL,
