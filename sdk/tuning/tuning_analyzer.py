...
class TuningAnalyzer:
    ...
    def analyze_step_response(
        ...
        metrics = self._calculate_time_domain_metrics(
            time_data, setpoint_data, response_data
        )
        ...
        if self.enable_plots:
            plot_path = self._plot_step_response(
                time_data, setpoint_data, response_data, control_data
            )
            if plot_path:
    ...
    def analyze_stability(
        ...
        plots = []
        if self.enable_plots:
            plot_path = self._plot_stability_analysis(
                time_data, response_data, control_data
            )
            if plot_path:
                plots.append(plot_path)
    ...
    def compare_configurations(
        ...
        config2_name: str = "Configuration 2",
    ) -> AnalysisResult:
        """
    ...
    def _calculate_time_domain_metrics(
        self,
        time_data: np.ndarray,
        setpoint_data: np.ndarray,
        response_data: np.ndarray,
        ...
        # Final value
        final_value = (
            np.mean(setpoint_data[-100:])
            if len(setpoint_data) > 100
            else setpoint_data[-1]
        )
        initial_value = (
            np.mean(response_data[: step_start + 1])
            if step_start > 0
        ...
        if len(settled_indices) > 0:
            ...
            for i in range(len(settled_indices) - 10):
                if np.all(
                    np.abs(
                        step_response[settled_indices[i] : settled_indices[i] + 10]
                        - final_value
                    )
                    <= tolerance
                ):
        ...
        if abs(value_range) > 1e-6:
            ...
            if value_range > 0:  # Positive step
                metrics.overshoot = max(
                    0, (max_response - final_value) / value_range * 100
                )
                metrics.undershoot = max(
                    0, (initial_value - min_response) / value_range * 100
                )
            else:  # Negative step
                metrics.overshoot = max(
                    0, (final_value - min_response) / abs(value_range) * 100
                )
                metrics.undershoot = max(
                    0, (max_response - initial_value) / abs(value_range) * 100
                )
    ...
    def _compare_metrics(
        ...
        if metrics1.rise_time > 0:
            comparison.rise_time = (
                (metrics1.rise_time - metrics2.rise_time) / metrics1.rise_time * 100
            )
        if metrics1.settling_time > 0:
            comparison.settling_time = (
                (metrics1.settling_time - metrics2.settling_time)
                / metrics1.settling_time
                * 100
            )
        if metrics1.overshoot > 0:
            comparison.overshoot = (
                (metrics1.overshoot - metrics2.overshoot) / metrics1.overshoot * 100
            )
        if metrics1.rms_error > 0:
            comparison.rms_error = (
                (metrics1.rms_error - metrics2.rms_error) / metrics1.rms_error * 100
            )
        if metrics1.control_effort > 0:
            comparison.control_effort = (
                (metrics1.control_effort - metrics2.control_effort)
    ...
    def _generate_step_response_recommendations(
        ...
        if metrics.rise_time > 2.0:
            ...
            recommendations.append(
                "Rise time is very fast. Consider reducing P gain to avoid overshoot."
            )

        # Settling time recommendations
        if metrics.settling_time > 5.0:
            recommendations.append(
                "Settling time is too long. Increase P gain or add D gain."
            )
        ...
        if metrics.overshoot > 20.0:
            recommendations.append(
                "Excessive overshoot. Reduce P gain or increase D gain."
            )
        ...
        if metrics.control_effort > 100.0:
            ...
            recommendations.append(
                "Control saturation detected. Reduce gains or increase limits."
            )

        return recommendations

    def _generate_frequency_response_recommendations(
        self, metrics: PerformanceMetrics
    ) -> List[str]:
        ...
        if metrics.phase_margin < 30.0:
            recommendations.append(
                "Low phase margin. System may be unstable or oscillatory."
            )
        ...
        if metrics.gain_margin < 6.0:
            recommendations.append(
                "Low gain margin. Reduce P gain for better stability."
            )
        elif metrics.gain_margin > 20.0:
            recommendations.append(
                "High gain margin. Could increase P gain for better performance."
            )
    ...
    def _generate_stability_recommendations(
        ...
        if metrics.oscillation_frequency > 10.0:
            recommendations.append(
                "High frequency oscillations detected. Reduce P gain or add filtering."
            )
        elif metrics.oscillation_frequency > 2.0:
            recommendations.append(
                "Moderate oscillations. Consider adjusting PID gains."
            )

        # Damping ratio recommendations
        if metrics.damping_ratio < 0.3:
            recommendations.append(
                "Underdamped system. Increase D gain or reduce P gain."
            )
        elif metrics.damping_ratio > 0.8:
            recommendations.append(
                "Overdamped system. Could increase P gain for faster response."
            )
    ...
    def _generate_comparison_recommendations(
        ...
        if metrics2.rise_time < metrics1.rise_time * 0.8:
            recommendations.append(
                f"{config2_name} has significantly faster rise time."
            )
        elif metrics2.rise_time > metrics1.rise_time * 1.2:
            recommendations.append(
                f"{config1_name} has significantly faster rise time."
            )

        # Overshoot comparison
        if metrics2.overshoot < metrics1.overshoot * 0.8:
            recommendations.append(
                f"{config2_name} has better overshoot characteristics."
            )
        elif metrics2.overshoot > metrics1.overshoot * 1.2:
            recommendations.append(
                f"{config1_name} has better overshoot characteristics."
            )
    ...
    def _plot_step_response(
        ...
        try:
            ...
            if control_data is not None:
                axes[1].plot(
                    time_data, control_data, "g-", label="Control Signal", linewidth=2
                )
    ...
    def _plot_frequency_response(
        self, frequencies: np.ndarray, magnitude_db: np.ndarray, phase: np.ndarray
    ) -> Optional[str]:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "analysis_types": [
                result.analysis_type.value for result in self.analysis_history
            ],
            "latest_analysis": (
                self.analysis_history[-1].to_dict() if self.analysis_history else None
            ),
        }
