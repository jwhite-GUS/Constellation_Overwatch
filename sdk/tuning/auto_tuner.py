...
class SystemIdentifier:
    ...
    @staticmethod
    def relay_feedback_identification(
        input_data: np.ndarray, output_data: np.ndarray, sample_time: float
    ) -> Dict:
        """
        ...
        peaks_pos, _ = signal.find_peaks(
            output_centered, height=0.1 * np.std(output_centered)
        )
        ...
        amplitude = np.mean(
            [
                np.mean(output_centered[peaks_pos]),
    ...
    @staticmethod
    def frequency_response_identification(
...
class AutoTuner:
    ...
    def start_tuning(
        ...
        output_callback: Callable[[float], None],
    ) -> TuningResult:
        """
        ...
        try:
            ...
            system_model = self._perform_system_identification(
                measurement_callback, output_callback
            )
            ...
            validation_result = self._validate_parameters(
                new_gains, measurement_callback, output_callback
            )
            ...
            self.logger.info(
                f"Auto-tuning completed successfully in {self.result.execution_time:.1f}s"
            )
    ...
    def _perform_system_identification(
        self,
        measurement_callback: Callable[[], float],
        output_callback: Callable[[float], None],
    ) -> Dict:
        ...
        if self.config.method == TuningMethod.RELAY_FEEDBACK:
            ...
            self.logger.warning(
                f"Method {self.config.method} not implemented, using relay feedback"
            )
            return self._relay_feedback_test(measurement_callback, output_callback)

    def _relay_feedback_test(
        self,
        measurement_callback: Callable[[], float],
        output_callback: Callable[[float], None],
    ) -> Dict:
        ...
        if identification_result["success"]:
            self.logger.info(
                f"Relay test successful: Ku={identification_result['ultimate_gain']:.3f}, "
                f"Tu={identification_result['ultimate_period']:.3f}s"
            )

        return identification_result

    def _sine_sweep_test(
        self,
        measurement_callback: Callable[[], float],
        output_callback: Callable[[float], None],
    ) -> Dict:
    ...
    def _step_response_test(
        ...
        while (time.time() - step_start_time) < test_time and not self._stop_requested:
            ...
            if not self.safety_monitor.is_safe(
                measurement, step_amplitude, current_time
            ):
                break
        ...
        final_value = (
            np.mean(output_array[-50:]) if len(output_array) > 50 else output_array[-1]
        )
        ...
        return {
            ...
            "overshoot": (
                max(step_response) - final_value if len(step_response) > 0 else 0
            ),
    ...
    def _calculate_ziegler_nichols(self, system_model: Dict) -> PIDGains:
        ...
        self.logger.info(
            f"Ziegler-Nichols PID gains: P={Kp:.4f}, I={Ki:.4f}, D={Kd:.4f}"
        )
    ...
    def _validate_parameters(
        ...
        while (time.time() - start_time) < test_time and not self._stop_requested:
            ...
            # Safety check
            if not self.safety_monitor.is_safe(
                measurement, control_output, current_time
            ):
        ...
        max_overshoot = (
            max(validation_data["output"]) - setpoint
            if validation_data["output"]
            else 0
        )
        ...
        validation_success = (
            final_error < 0.1
            and max_overshoot < setpoint * 0.3  # Good steady-state accuracy
            and oscillation_count < 10  # Limited overshoot
            and (  # Stable response
                settling_time is None or settling_time < test_time * 0.8
            )  # Reasonable settling
        )
        ...
        if validation_success:
            ...
            self.logger.info(
                f"Validation successful: final_error={final_error:.3f}, "
                f"overshoot={max_overshoot:.3f}"
            )
        else:
            self.logger.warning(
                f"Validation failed: final_error={final_error:.3f}, "
