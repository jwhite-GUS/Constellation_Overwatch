...
from sdk.core.message_bus import (
    MessageBus,
    MessageType,
    ...
    async def _publish_telemetry(self):
        ...
        while self.running:
            await publish_vehicle_telemetry(
                self.message_bus, self.vehicle_id, self.telemetry.copy()
            )
...
class SimpleGroundControl:
    ...
    async def start(self):
        ...
        await self.message_bus.subscribe(
            callback=self._handle_telemetry, topics={"telemetry"}
        )

        # Subscribe to entity events
        await self.message_bus.subscribe(
            callback=self._handle_entity_event, topics={"entities"}
        )
    ...
    async def _handle_telemetry(self, message):
        ...
        print(
            f"GCS: Telemetry from {vehicle_id} - "
    ...
    async def send_command(
        self, vehicle_id: str, command: str, params: Dict[str, Any] = None
    ):
...
async def demo_functional_core():
    ...
    async def entity_event_publisher(event_type: str, entity):
        await publish_entity_event(
            message_bus, event_type, entity.entity_id, entity.to_dict()
        )
    ...
    # Navigation commands
    await gcs.send_command(
        drone1.entity_id, "goto", {"waypoint": {"lat": 40.7200, "lon": -74.0100}}
    )
    await asyncio.sleep(3)

    await gcs.send_command(
        drone2.entity_id, "goto", {"waypoint": {"lat": 40.7600, "lon": -73.9800}}
    )
    await asyncio.sleep(3)
