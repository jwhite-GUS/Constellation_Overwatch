    ...
    async def goto_position(self, latitude: float, longitude: float, altitude: float):
        ...
        for i in range(steps):
            ...
            self.position.latitude = (
                start_pos.latitude + (latitude - start_pos.latitude) * progress
            )
            self.position.longitude = (
                start_pos.longitude + (longitude - start_pos.longitude) * progress
            )
            self.position.altitude = (
                start_pos.altitude + (altitude - start_pos.altitude) * progress
            )
            await asyncio.sleep(0.1)

        logger.info(
            f"Arrived at position: {self.position.latitude}, {self.position.longitude}, {self.position.altitude}m"
        )
...
class CameraSensor(SensorInterface):
    ...
    async def initialize(self) -> bool:
        ...
        logger.info(
            f"Camera sensor {self.sensor_id} initialized - {self.resolution} @ {self.fps}fps"
        )
        return True
...
class GimbalPayload(PayloadInterface):
    ...
    async def scan_area(self):
        ...
        scan_positions = [(-45, 0), (0, 0), (45, 0), (-45, -30), (0, -30), (45, -30)]
...
async def main():
    ...
    config = {"simulation": True, "log_level": "INFO"}
    ...
    await drone.send_command(
        "goto", {"latitude": 37.7849, "longitude": -122.4094, "altitude": 30.0}
    )
