...
class ParameterManager:
    ...
    def _initialize_default_parameters(self):
        ...
        control_set = ParameterSet(
            name="flight_control",
            description="Primary flight control parameters"
        )
        ...
        for axis in axes:
            ...
            control_set.add_parameter(Parameter(
        ...
        safety_set = ParameterSet(
            name="safety",
            description="Safety and limit parameters"
        )
        ...
        safety_params = [
            ...
            ("SAFETY_OSCILLATION_FREQ", 5.0, "Oscillation detection frequency", "Hz", 1.0, 20.0),
        ]
        
        for name, value, desc, units, min_val, max_val in safety_params:
            safety_set.add_parameter(Parameter(
                name=name,
                value=value,
                param_type=ParameterType.FLOAT,
        ...
        tuning_set = ParameterSet(
            name="tuning",
            description="Auto-tuning configuration parameters"
        )
        
        tuning_params = [
            ("TUNE_AGGR", 0.075, ParameterType.FLOAT, "Tuning aggressiveness", 0.05, 0.15),
            ("TUNE_METHOD", "relay_feedback", ParameterType.STRING, "Tuning method", None, None),
            ("TUNE_TIMEOUT", 60.0, ParameterType.FLOAT, "Tuning timeout", 10.0, 300.0),
            ("TUNE_ENABLE_ROLL", True, ParameterType.BOOL, "Enable roll tuning", None, None),
            ("TUNE_ENABLE_PITCH", True, ParameterType.BOOL, "Enable pitch tuning", None, None),
            ("TUNE_ENABLE_YAW", True, ParameterType.BOOL, "Enable yaw tuning", None, None),
        ]
        
        for name, value, param_type, desc, min_val, max_val in tuning_params:
            tuning_set.add_parameter(Parameter(
                name=name,
                value=value,
    ...
    def get_parameter(self, name: str, set_name: Optional[str] = None) -> Optional[Parameter]:
        """Get parameter by name"""
    ...
    def set_parameter(self, name: str, value: Any, set_name: Optional[str] = None) -> bool:
    ...
    def load_gains_from_pid(self, gains: Dict[str, PIDGains], set_name: Optional[str] = None):
    ...
    def export_gains_to_pid(self, set_name: Optional[str] = None) -> Dict[str, PIDGains]:
    ...
    def backup_parameters(self, backup_name: str = None) -> str:
        ...
        backup_data = {
            ...
            'parameter_sets': {name: param_set.to_dict() for name, param_set in self.parameter_sets.items()}
        }
    ...
    def load_configuration(self, config_path: str):
        ...
        try:
            ...
            if config_data.get('version') != '1.0':
                self.logger.warning(f"Unknown configuration version: {config_data.get('version')}")
    ...
    def get_parameters_by_category(self, category: ParameterCategory, 
                                 set_name: Optional[str] = None) -> Dict[str, Parameter]:
        ...
        param_set = self.parameter_sets[target_set]
        return {name: param for name, param in param_set.parameters.items() 
    ...
    def search_parameters(self, search_term: str, 
        ...
        for name, param in param_set.parameters.items():
            if (search_term.lower() in name.lower() or 
                search_term.lower() in param.description.lower()):
                results[name] = param
                
        return results
        
    def validate_all_parameters(self, set_name: Optional[str] = None) -> Dict[str, List[str]]:
        ...
        for name, param in param_set.parameters.items():
            ...
            if param.param_type == ParameterType.FLOAT:
                if param.min_value is not None and param.value < param.min_value:
                    errors.append(f"Value {param.value} below minimum {param.min_value}")
                if param.max_value is not None and param.value > param.max_value:
                    errors.append(f"Value {param.value} above maximum {param.max_value}")
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            'total_parameters': sum(len(ps.parameters) for ps in self.parameter_sets.values()),
            'recent_changes': len([c for c in self.change_history if time.time() - c['timestamp'] < 3600]),
