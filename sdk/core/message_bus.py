...
class MessageBus:
    ...
    async def subscribe(
        ...
        print(
            f"Added subscriber {subscriber.subscriber_id[:8]}... "
...
async def publish_entity_event(
    bus: MessageBus, event_type: str, entity_id: str, entity_data: Dict[str, Any]
):
...
async def publish_vehicle_telemetry(
    bus: MessageBus, vehicle_id: str, telemetry: Dict[str, Any]
):
...
async def publish_mission_command(
    bus: MessageBus, source: str, target_vehicle: str, command: Dict[str, Any]
):
...
async def main():
    ...
    telemetry_subscriber_id = await bus.subscribe(
        callback=example_subscriber, topics={"telemetry"}
    )
    ...
    await publish_vehicle_telemetry(
        bus, "drone-001", {"battery": 85, "altitude": 100, "speed": 5.2}
    )
