...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Train AI models for Constellation Overwatch SDK")
    parser.add_argument("--model-type", choices=["vision", "decision", "nlp", "all"], 
    ...
    if args.model_type in ["vision", "all"]:
        ...
        cv_trainer = ComputerVisionTrainer(os.path.join(args.output_dir, "computer_vision"))
    ...
    if args.model_type in ["decision", "all"]:
        ...
        dm_trainer = DecisionMakingTrainer(os.path.join(args.output_dir, "decision_making"))
        await dm_trainer.train_reinforcement_learning(config["decision"]["reinforcement_learning"])
    ...
    if args.model_type in ["nlp", "all"]:
        ...
        await nlp_trainer.train_command_interpreter(config["nlp"]["command_interpreter"])
