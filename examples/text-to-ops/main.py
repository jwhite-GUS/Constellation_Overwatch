...
class AdvancedNLPProcessor(NaturalLanguageInterface):
    ...
    async def parse_command_with_context(self, text: str, context: ConversationContext) -> ParsedCommand:
        ...
        for cmd_type, cmd_config in self.command_patterns.items():
            for pattern in cmd_config["patterns"]:
                if re.search(pattern, text_clean):
                    ...
                    confidence = 0.8 + (0.1 if cmd_config.get("context_dependent") else 0.0)
                    break
            if command_type != CommandType.UNKNOWN:
                break
        
        # Extract parameters based on command type
        parameters = await self.extract_command_parameters(text_clean, command_type, entities, context)
    ...
    async def extract_entities_advanced(self, text: str) -> Dict[str, Any]:
            ...
            if matches:
                if entity_type == "coordinates":
                    ...
                    for match in matches:
                        if len(match) == 2:
                            coords.append({"lat": float(match[0]), "lon": float(match[1])})
        ...
        location_keywords = [
            "base", "home", "target", "waypoint", "checkpoint",
            "building", "tower", "bridge", "park", "field",
    ...
    async def extract_command_parameters(self, text: str, command_type: CommandType, 
        ...
        if command_type == CommandType.TAKEOFF:
            ...
            if "coordinates" in entities:
                ...
                parameters["destination"] = await self.resolve_location_name(location, context)
    ...
    async def resolve_location_name(self, location_name: str, context: ConversationContext) -> Dict[str, float]:
        ...
        return known_locations.get(location_name, {"latitude": 37.7749, "longitude": -122.4194})
    
    def extract_drone_targets(self, text: str, entities: Dict[str, Any], context: ConversationContext) -> List[str]:
        ...
        if "all" in text or "everyone" in text or "entire" in text:
            ...
            targets = [drone_id for drone_id, state in context.drone_states.items() 
    ...
    def determine_priority(self, text: str, command_type: CommandType) -> str:
        ...
        if any(keyword in text for keyword in emergency_keywords):
            ...
            return "high"
        elif command_type in [CommandType.TAKEOFF, CommandType.LAND, CommandType.RETURN]:
            return "medium"
        else:
            return "low"
    
    def estimate_command_duration(self, command_type: CommandType, parameters: Dict[str, Any]) -> float:
...
class ConversationalDroneController:
    ...
    async def initialize(self) -> bool:
        ...
        logger.info(f"Initializing conversational drone controller: {self.controller_id}")
    ...
    async def start_conversation(self, operator_id: str) -> str:
        ...
        context = ConversationContext(
            ...
            mission_context={},
            recent_commands=[],
            drone_states={drone_id: {"status": "ready", "in_formation": False} 
        ...
        logger.info(f"Started conversation {conversation_id} with operator {operator_id}")
        return welcome_message
    
    async def process_natural_language_command(self, conversation_id: str, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            parsed_command = await self.nlp_processor.parse_command_with_context(text, context)
            
            # Log the parsed command
            logger.info(f"Parsed command: {parsed_command.command_type.value} "
                       f"(confidence: {parsed_command.confidence:.2f})")
            
            # Execute command
            execution_result = await self.execute_parsed_command(parsed_command, context)
            
            # Generate response
            response = await self.generate_response(parsed_command, execution_result, context)
    ...
    async def execute_parsed_command(self, command: ParsedCommand, context: ConversationContext) -> Dict[str, Any]:
        ...
        for drone_id in command.drone_targets:
            ...
            try:
                ...
                if command.command_type == CommandType.TAKEOFF:
                    ...
                    success = await drone_system.send_command("takeoff", {"altitude": altitude})
                    results[drone_id] = {"success": success, "action": "takeoff", "altitude": altitude}
                
                elif command.command_type == CommandType.LAND:
                    success = await drone_system.send_command("land", command.parameters)
                    ...
                    altitude = command.parameters.get("altitude", 50.0)
                    success = await drone_system.send_command("goto", {
                        "position": destination,
                        "altitude": altitude
                    })
                    results[drone_id] = {"success": success, "action": "goto", "destination": destination}
                    ...
                    results[drone_id] = {"success": success, "action": "search", "parameters": search_params}
                    ...
                    results[drone_id] = {"success": success, "action": "patrol", "parameters": patrol_params}
                    ...
                    results[drone_id] = {"success": True, "action": "status", "data": telemetry}
    ...
    async def generate_response(self, command: ParsedCommand, execution_result: Dict[str, Any], 
                              context: ConversationContext) -> str:
        """Generate natural language response"""
        success_count = sum(1 for result in execution_result.values() 
        ...
        if command.command_type == CommandType.TAKEOFF:
            ...
            duration = command.parameters.get("duration", 300) / 60  # Convert to minutes
            if success_count == total_count:
                return f"All {total_count} drone(s) are starting patrol mission for {duration:.1f} minutes."
            else:
                return f"{success_count} of {total_count} drone(s) have begun patrolling."
            ...
            if success_count > 0:
                return f"Command executed for {success_count} of {total_count} drone(s)."
            else:
...
async def main():
    ...
    for i, command in enumerate(test_commands):
        ...
        result = await controller.process_natural_language_command(conversation_id, command)
        ...
        if result.get("success"):
            ...
            print(f"  └─ Parsed as: {cmd_info['type']} (confidence: {cmd_info['confidence']:.2f})")
