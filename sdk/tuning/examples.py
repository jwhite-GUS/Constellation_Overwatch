...
class TuningExamples:
    ...
    async def example_parameter_comparison(self) -> Dict[str, float]:
        ...
        conservative_params = ParameterSet(
            name="conservative", description="Conservative tuning parameters"
        )
        conservative_params.add_parameter(
            "pid_roll_p", 0.3, 0.1, 1.0, "Conservative P gain"
        )
        conservative_params.add_parameter(
            "pid_roll_i", 0.05, 0.0, 0.5, "Conservative I gain"
        )
        conservative_params.add_parameter(
            "pid_roll_d", 0.02, 0.0, 0.2, "Conservative D gain"
        )

        aggressive_params = ParameterSet(
            name="aggressive", description="Aggressive tuning parameters"
        )
        aggressive_params.add_parameter(
            "pid_roll_p", 0.8, 0.1, 1.0, "Aggressive P gain"
        )
        aggressive_params.add_parameter(
            "pid_roll_i", 0.2, 0.0, 0.5, "Aggressive I gain"
        )
        aggressive_params.add_parameter(
            "pid_roll_d", 0.1, 0.0, 0.2, "Aggressive D gain"
        )
        ...
        comparison_result = self.analyzer.compare_configurations(
            conservative_data, aggressive_data, "Conservative", "Aggressive"
        )
    ...
    async def example_safety_monitoring(self) -> Dict[str, bool]:
        ...
        unstable_params = ParameterSet(
            name="unstable", description="Intentionally unstable parameters"
        )
        ...
        for i in range(len(time_data)):
            ...
            if violations:
                ...
                logger.warning(
                    f"Safety violation at t={time_data[i]:.2f}s: {violations}"
                )
    ...
    async def example_automated_workflow(self) -> str:
        ...
        report_path = self.analyzer.generate_report(
            f"automated_tuning_report_{session_id}.html"
        )
    ...
    async def _simulate_response(
        ...
        for i in range(len(time_data)):
            if i > 0:
                ...
                acceleration = (
                    wn**2 * (control[i - 1] - response[i - 1])
                    - 2 * zeta * wn * (response[i] - response[i - 1]) / dt
                )
