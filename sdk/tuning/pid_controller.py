...
class PIDController:
    ...
    def update(
        self, setpoint: float, measurement: float, dt: Optional[float] = None
    ) -> float:
        """
        ...
        if self.form == PIDForm.PARALLEL:
            ...
            integral_component = (
                (self.gains.K / self.gains.Ti) * self.state.integral
                if self.gains.Ti > 0
                else 0
            )
    ...
    def _update_integral(self, error: float, dt: float) -> float:
        ...
        new_integral = np.clip(
            new_integral, self.gains.integral_min, self.gains.integral_max
        )
    ...
    def _monitor_performance(self, error: float, output: float, dt: float):
        ...
        self.performance_history.append(
            {
                "time": (
                    self.performance_history[-1]["time"] + dt
    ...
    def get_performance_metrics(self) -> Dict:
        ...
        oscillation_frequency = (
            self.oscillation_count / (len(self.performance_history) * self.sample_time)
            if self.performance_history
            else 0
        )
        ...
        return {
            ...
            "output_saturation": sum(1 for o in outputs if abs(o) >= 0.95)
            / len(outputs),
        }
    ...
    def is_stable(self) -> bool:
        ...
        # Stability criteria
        stable = (
            metrics.get("oscillation_frequency", 0) < 2.0
            and metrics.get("output_saturation", 0) < 0.1  # Less than 2 Hz oscillation
            and metrics.get("rms_error", float("inf"))  # Less than 10% saturation
            < 0.2  # RMS error less than 0.2
        )
