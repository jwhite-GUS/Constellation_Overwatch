...
class VRGestureRecognizer:
    ...
    async def recognize_gesture(
        self, left_controller: VRControllerInput, right_controller: VRControllerInput
    ) -> Optional[VRGestureCommand]:
        ...
        self.gesture_history = [
            h for h in self.gesture_history if h["timestamp"] > cutoff_time
        ]

        # Check each gesture template
        for gesture_name, template in self.gesture_templates.items():
            confidence = await self.match_gesture_pattern(
                template, self.gesture_history
            )

            if confidence > self.recognition_threshold:
                # Extract parameters based on gesture
                parameters = await self.extract_gesture_parameters(
                    gesture_name, self.gesture_history
                )
    ...
    async def match_gesture_pattern(
        self, template: Dict[str, Any], history: List[Dict]
    ) -> float:
        ...
        if pattern == "point_trigger":
            ...
            if len(history) >= 10:
                ...
                early_distance = self.calculate_hand_distance(
                    early["left"], early["right"]
                )
                recent_distance = self.calculate_hand_distance(
            ...
            if len(history) >= 8:
                positions = [
                    h["right"].position[1] for h in history[-8:]
                ]  # Y positions
            ...
            if len(history) >= 15:
                positions = [
                    (h["right"].position[0], h["right"].position[2])
                    for h in history[-15:]
                ]
    ...
    def calculate_hand_distance(
        self, left_ctrl: VRControllerInput, right_ctrl: VRControllerInput
    ) -> float:
    ...
    def is_circular_motion(self, positions: List[Tuple[float, float]]) -> bool:
        ...
        distances = [
            math.sqrt((pos[0] - center_x) ** 2 + (pos[1] - center_z) ** 2)
    ...
    async def extract_gesture_parameters(
        self, gesture_name: str, history: List[Dict]
    ) -> Dict[str, Any]:
        ...
        if gesture_name == "point_and_send":
            ...
            positions = [
                (h["right"].position[0], h["right"].position[2]) for h in history[-15:]
            ]
            center_x = sum(pos[0] for pos in positions) / len(positions)
            center_z = sum(pos[1] for pos in positions) / len(positions)
...
class VRDroneController:
    ...
    async def register_drone_system(self, drone_system: AutonomousSystem):
        ...
        await self.vr_interface.load_drone_model(
            drone_system.system_id,
            {
    ...
    async def execute_gesture_command(self, gesture_command: VRGestureCommand):
        ...
        logger.info(
            f"Executing gesture command: {gesture_command.gesture_type} "
        ...
        target_drones = (
            ...
            else list(self.drone_systems.keys())
        )
        ...
        if gesture_command.gesture_type == "point_and_send":
            ...
            for drone_id in target_drones:
                if drone_id in self.drone_systems:
                    await self.drone_systems[drone_id].send_command(
                        "goto", {"position": world_pos}
                    )

        elif gesture_command.gesture_type == "formation_spread":
            # Spread formation
            spread_distance = (
                gesture_command.parameters["distance"] * 10
            )  # Scale to meters
            ...
            # Contract formation
            contract_distance = (
                gesture_command.parameters["distance"] * 5
            )  # Scale to meters
            ...
            for drone_id in target_drones:
                if drone_id in self.drone_systems:
                    await self.drone_systems[drone_id].send_command(
                        "land", {"position": world_pos}
                    )

    async def process_button_input(
        self, left_controller: VRControllerInput, right_controller: VRControllerInput
    ):
    ...
    async def update_vr_display(self):
        ...
        for drone_id, vr_drone in self.vr_drones.items():
            if vr_drone.trail_points:
                await self.vr_interface.update_drone_trail(
                    drone_id, vr_drone.trail_points
                )
...
async def main():
    ...
    config = {"simulation": True, "vr_enabled": True, "graphics_quality": "high"}
